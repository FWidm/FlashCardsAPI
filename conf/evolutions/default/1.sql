...
alter table answer add constraint fk_answer_card_2 foreign key (card_flashcard_id) references flash_card (flashcard_id) on delete restrict on update restrict;
create index ix_answer_card_2 on answer (card_flashcard_id);
...
alter table question add constraint fk_question_author_4 foreign key (author_id) references user (id) on delete restrict on update restrict;
create index ix_question_author_4 on question (author_id);
alter table question add constraint fk_question_card_5 foreign key (card_flashcard_id) references flash_card (flashcard_id) on delete restrict on update restrict;
create index ix_question_card_5 on question (card_flashcard_id);
